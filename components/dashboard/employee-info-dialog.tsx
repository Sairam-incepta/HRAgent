...
import { Clock, BarChart, FileText, TrendingUp, DollarSign, ArrowLeft, ArrowRight, Edit, Star } from "lucide-react";
...
export function EmployeeInfoDialog({
    ...
    const maxDailyHours = useMemo(() => {
        ...
        return Math.max(...allDailyHours, 12);
    }, [weeklyData]);
    ...
    const goNext = () => {
        ...
        if (nextStartStr > todayStr) return;
    ...
    useEffect(() => {
        ...
        const cleanupFunctions = [
            ...
            dashboardEvents.on('policy_sales_updated', handlePolicySaleUpdate),
    ...
    const loadWeekData = async () => {
        ...
        try {
            let weekly;
            if (currentWeekStart) {
                weekly = await getPeriodSummary(user.id, currentWeekStart, 7);
            } else {
                weekly = await getWeeklySummary(user.id);
            }
    ...
    const calculateAllPolicyBonuses = async (policies: any[]) => {
        if (!policies.length || !user?.id) return;

        setBonusesLoading(true);
        try {
            const bonusPromises = policies.map(async (policy) => {
                const bonusData = await calculateIndividualPolicyBonus(policy.id, user.id);
        ...
        <>
            <Dialog open={open} onOpenChange={onOpenChange}>
                <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                    <DialogHeader>
                        <DialogTitle>My Employee Information</DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="space-y-6">
                        <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                                ...
                                <p className="text-muted-foreground">{employeeData.email}</p>
                        ...
                        <Tabs defaultValue="overview" className="space-y-4">
                            ...
                            <TabsContent value="overview" className="space-y-4">
                                        ...
                                        <div className="bg-card rounded-lg border p-4 hover:shadow-sm transition-shadow h-20">
                                            <div className="flex items-center justify-between h-full">
                                                ...
                                                <div className="h-8 w-8 bg-blue-100 dark:bg-blue-900/30 rounded-lg flex items-center justify-center flex-shrink-0 ml-3">
                                                    <Star className="h-4 w-4 text-[#005cb3] dark:text-blue-400" />
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <Card>
                                    <CardHeader>
                                        <CardTitle>Recent Activity & Weekly Hours</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        {loading ? (
                                                                ...
                                                                <div className="space-y-4">
                                                                    ...
                                                                    <div className="space-y-2">
                                                                        ...
                                                                        <div className="relative w-full bg-gray-200 rounded-full h-4">
                                                                            ...
                                                                            {weeklyOvertime > 0 && (
                                                                                <div
                                                                                    className="absolute top-0 left-0 bg-orange-500 h-4 rounded-full transition-all duration-500 opacity-80"
                                    ...
                                    <CardContent>
                                        {loading ? (
                                            ...
                                            <div className="text-center py-8 text-muted-foreground">
                                                ...
                                                <p className="text-sm mt-2">Daily summaries are created when you submit your end-of-day reports through the chat interface.</p>
                                            </div>
                                        )}
                                    </CardContent>
                                </Card>
                            ...
                            <TabsContent value="policies" className="space-y-4">
                                <Card>
                                    <CardHeader>
                                        <CardTitle>My Policies ({totalPolicies})</CardTitle>
                                    </CardHeader>
                                    <CardContent>
                                        {loading ? (
                                            <div className="space-y-4">
                                            ...
                                            <div className="space-y-4">
                                                {employeePolicies.map((policy, index) => (
                                                    <div key={index} className="border rounded-lg p-4 space-y-3">
                                                        ...
                                                        {policy.is_cross_sold_policy && (policy.cross_sold_type || policy.cross_sold_to) && (
                                                            <div className="grid grid-cols-2 gap-4 text-sm">
                                                                {policy.cross_sold_type && (
                                                                    <div>
                                                                        <span className="text-muted-foreground">Cross-sold Type:</span>
                                                                        <span className="ml-2 font-medium">{policy.cross_sold_type}</span>
                                                                    </div>
                                                                )}
                                                                {policy.cross_sold_to && (
                                                                    <div>
                                                                        <span className="text-muted-foreground">Cross-sold To:</span>
                                                                        <span className="ml-2 font-medium">{policy.cross_sold_to}</span>
                                                                    </div>
                                                                )}
                                                            </div>
                                                        )}
                                            ...
                                            <div className="text-center py-8 text-muted-foreground">
                                                ...
                                                <p className="text-sm mt-2">Your sold policies will appear here once you start making sales.</p>
                                            </div>
                                        )}
                                    </CardContent>
                                </Card>
                                    ...
                                    <CardContent>
                                        {loading ? (
                                                    ...
                                                    <div key={index} className="border rounded-lg p-4 space-y-3">
                                                        <div className="flex justify-between items-start">
                                                            ...
                                                            <div className="text-right">
                                                                ...
                                                                {review.rating === 5 && (
                                                                    <Badge variant="outline" className="text-xs bg-yellow-100 text-yellow-800 dark:bg-yellow-900/30 dark:text-yellow-400 mt-1">
                                                                        +$10 Bonus
                                                                    </Badge>
                                                                )}
                                                            </div>
                                            ...
                                            <div className="text-center py-8 text-muted-foreground">
                                                ...
                                                <p className="text-sm mt-2">Client reviews will appear here once your clients submit feedback.</p>
                                                <p className="text-sm mt-1 text-[#005cb3]">5-star reviews earn you a $10 bonus!</p>
                                            </div>
                                        )}
                                    </CardContent>
                                </Card>
